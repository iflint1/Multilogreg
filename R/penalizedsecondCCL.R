...
#' @param covariate Covariates. Can be either a raster image or a matrix.
...
#' @param prints Optional. Printing value of likelihood function along with convergence rate for each iteration.
...
PenalizedSecondOrderCCL <- function(X=NULL,psi0=NULL,xi0=NULL,sigma0=NULL,phi0=NULL,Beta,xibound=c(1e-5,Inf),sigmabound=c(1e-5,Inf),phibound=c(1e-5,Inf),lamb=0,lat=1,Rmax=NULL,covariate=NULL,ite=100,tol=1e-5,tol.alpha=1e-10,tol.eta=1e-10,mu=1, prints = T){
  ...
  for(n in 1:nlat){
    ...
    psi.final = array(alpha0,c(p-1,q,nlamb+1))
    ...
    result$psi    = array(psi.final[,,1:nlamb],c(p-1,q,nlamb))
