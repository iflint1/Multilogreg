#' @title Gradient of sigma^2
#' @description Gradient of sigma^2
...
#' @return gradient of sigma^2
#' @export

gradient.sigma2 <- function(alphas,xis,sigmas,phis,R,fi,fj,mi,mj,mval){
  p=length(mval)
  ...
  for(k1 in 1:p){
    for(k2 in 1:p){
      ...
      if(k1==k2){
        ...
        ptmp_grad[k1] <- ptmp_grad[k1] + sum(exp(-R_sub/exp(phis[k1])))
        ptmp_grad2[k1] <- ptmp_grad2[k1] + sum(tmp3/aggPtmp)
